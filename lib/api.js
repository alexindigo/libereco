...
apiBase.prototype.getAccessToken = function apiBaseGetAccessToken(token, secret, verifier, callback)
{
    ...
    this.OA.getOAuthAccessToken(token, secret, verifier, function oauthAccessTokenHandler(err, access_token, access_secret, results)
    {
      ...
      this.OAuth.access_secret = access_secret;
      this.OAuth.user          = results;

      callback(null, access_token, access_secret, results);
    }.bind(this));
}

apiBase.prototype.fetchPhotos = function apiBaseFetchPhotos(callback)
{
  if (!this.Data.user)
  {
...
apiBase.prototype.fetchUser = function apiBaseFetchUser(callback)
{
  ...
  if (!this.OAuth.access_token) return console.error(['Lost access token', this.OAuth]);
...
{
  this.OA.get(url, this.OAuth.access_token, this.OAuth.access_secret, callback);
}

apiBase.prototype.apiPost = function apiBaseApiPost(url, callback)
{
  this.OA.post(url, this.OAuth.access_token, this.OAuth.access_secret, null, 'application/json', callback);
}
